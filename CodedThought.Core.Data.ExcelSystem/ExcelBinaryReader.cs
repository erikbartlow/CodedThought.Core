...
using CodedThought.Core.Data.ExcelSystem.Core.BinaryFormat;
using System.Data;
using System.Text;

namespace CodedThought.Core.Data.ExcelSystem {

	/// <summary>ExcelDataReader Class</summary>
	public class ExcelBinaryReader : IExcelDataReader {
		...
		private void readWorkBookGlobals() {
			...
			XlsRootDirectory dir = new(m_hdr);
			XlsDirectoryEntry workbookEntry = dir.FindEntry(WORKBOOK) ?? dir.FindEntry(BOOK);

			if (workbookEntry == null) { fail(Errors.ErrorStreamWorkbookNotFound); return; }

			if (workbookEntry.EntryType != STGTY.STGTY_STREAM) { fail(Errors.ErrorWorkbookIsNotStream); return; }
			...
			if (bof == null || bof.Type != BIFFTYPE.WorkbookGlobals) { fail(Errors.ErrorWorkbookGlobalsInvalidData); return; }
		...
		private DataTable readWholeWorkSheet(XlsWorksheet sheet) {
			...
			DataTable table = new(sheet.Name);
			...
			switch (format) {
				...
				default:
					...
					if (m_globals.Formats.TryGetValue(format, out fmtString)) {
						...
						FormatReader formatReader = new() { FormatString = fmt };
		...
		private object tryConvertOADateTime(object value, ushort XFormat) {
			...
			return double.TryParse(value.ToString(), out _dValue) ? tryConvertOADateTime(_dValue, XFormat) : value;
		}

		public bool isV8() => m_version >= 0x600;
		...
		public void Initialize(Stream fileStream) {
			...
			// set the sheet index to the index of the first sheet.. this is so that properties such as Name which use m_sheetIndex reflect the first sheet in the file without having to perform a
			// read() operation
			m_SheetIndex = 0;
		}

		public DataSet AsDataSet() => AsDataSet(false, true);
		...
		public string ExceptionMessage => m_exceptionMessage;

		public string Name {
			get {
				return null != m_sheets && m_sheets.Count > 0 ? m_sheets[m_SheetIndex].Name : null;
			}
		}
		...
		public int FieldCount => m_maxCol;

		public bool GetBoolean(int i) {
			return !IsDBNull(i) && Boolean.Parse(m_cellsValues[i].ToString());
		}
		...
		public object GetValue(int i) => m_cellsValues[i];

		public bool IsDBNull(int i) => (null == m_cellsValues[i]) || (DBNull.Value == m_cellsValues[i]);
		...
		public bool ConvertOaDate {
			get => m_ConvertOADate; set => m_ConvertOADate = value;
		}

		public ReadOption ReadOption => m_ReadOption;
