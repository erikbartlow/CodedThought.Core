...
namespace CodedThought.Core.Data.ExcelSystem.Core.BinaryFormat {
	...
	/// <summary>Represents a BIFF stream</summary>
	internal class XlsBiffStream : XlsStream {
		...
		/// <summary>Returns size of BIFF stream in bytes</summary>
		public int Size => m_size;

		/// <summary>Returns current position in BIFF stream</summary>
		public int Position => m_offset;

		//TODO:Remove ReadStream
		/// <summary>Always returns null, use biff-specific methods</summary>
		/// <returns></returns>
		[Obsolete("Use BIFF-specific methods for this stream")]
		public new byte[] ReadStream() => bytes;

		/// <summary>Sets stream pointer to the specified offset</summary>
		...
		/// <summary>Reads record under cursor and advances cursor position to next record</summary>
		/// <returns></returns>
		[MethodImpl(MethodImplOptions.Synchronized)]
		public XlsBiffRecord Read() {
			...
			m_offset += rec.Size;
			return m_offset > m_size ? null : rec;
		}

		/// <summary>Reads record at specified offset, does not change cursor position</summary>
