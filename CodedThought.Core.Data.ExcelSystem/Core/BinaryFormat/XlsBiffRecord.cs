namespace CodedThought.Core.Data.ExcelSystem.Core.BinaryFormat {
	...
	/// <summary>Represents basic BIFF record Base class for all BIFF record types</summary>
	internal class XlsBiffRecord {
		...
		/// <summary>Returns type ID of this entry</summary>
		public BIFFRECORDTYPE ID => (BIFFRECORDTYPE)BitConverter.ToUInt16(m_bytes, m_readoffset - 4);

		/// <summary>Returns data size of this entry</summary>
		public ushort RecordSize => BitConverter.ToUInt16(m_bytes, m_readoffset - 2);

		/// <summary>Returns whole size of structure</summary>
		public int Size => 4 + RecordSize;

		/// <summary>Returns record at specified offset</summary>
		...
		public bool IsCell {
			get {//FORMULA / Blank / MulBlank / RK / MulRk / BoolErr / Number / LabelSst
				bool isCell = false;
				switch ((BIFFRECORDTYPE)ID) {
					case BIFFRECORDTYPE.FORMULA:
					case BIFFRECORDTYPE.BLANK:
		...
		public byte ReadByte(int offset) => Buffer.GetByte(m_bytes, m_readoffset + offset);

		public ushort ReadUInt16(int offset) => BitConverter.ToUInt16(m_bytes, m_readoffset + offset);

		public uint ReadUInt32(int offset) => BitConverter.ToUInt32(m_bytes, m_readoffset + offset);

		public ulong ReadUInt64(int offset) => BitConverter.ToUInt64(m_bytes, m_readoffset + offset);

		public short ReadInt16(int offset) => BitConverter.ToInt16(m_bytes, m_readoffset + offset);

		public int ReadInt32(int offset) => BitConverter.ToInt32(m_bytes, m_readoffset + offset);

		public long ReadInt64(int offset) => BitConverter.ToInt64(m_bytes, m_readoffset + offset);
		...
		public float ReadFloat(int offset) => BitConverter.ToSingle(m_bytes, m_readoffset + offset);

		public double ReadDouble(int offset) => BitConverter.ToDouble(m_bytes, m_readoffset + offset);
	}
}