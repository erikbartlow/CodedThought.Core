...
{
    /// <summary>Represents an Excel file stream</summary>
    internal class XlsStream
    {
        ...
        /// <summary>Returns offset of first stream sector</summary>
        public uint BaseOffset => (uint) ((m_startSector + 1) * m_hdr.SectorSize);

        /// <summary>Returns number of first stream sector</summary>
        public uint BaseSector => (m_startSector);

        /// <summary>Reads stream data from file</summary>
        ...
        public byte[] ReadStream()
        {
            ...
            using (MemoryStream ms = new(sectorSize * 8))
            {
                lock (m_fileStream)
                {
                    do
