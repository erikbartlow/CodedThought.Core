...
{
    /// <summary>Helpers class</summary>
    internal static class Helpers
    {
#if CF_DEBUG || CF_RELEASE

		/// <summary>Determines whether [is single byte] [the specified encoding].</summary>
		/// <param name="encoding">The encoding.</param>
		/// <returns><c>true</c> if [is single byte] [the specified encoding]; otherwise, <c>false</c>.</returns>
        ...
        public static double Int64BitsToDouble(long value) => BitConverter.ToDouble(BitConverter.GetBytes(value), 0);

        private static Regex re = new("_x([0-9A-F]{4,4})_");

        public static string ConvertEscapeChars(string input) => re.Replace(input, m => (((char) UInt32.Parse(m.Groups[1].Value, NumberStyles.HexNumber))).ToString());
        ...
        {
            List<DataTable> tables = new(dataset.Tables.Count);
            bool convert = false;
