...
namespace CodedThought.Core.Data {
    ...
    public class GenericDataStore : IDBStore {
        ...
        protected virtual IDatabaseObject? DatabaseObjectInstance { get; set; }
        ...
        public virtual string? DefaultSchemaName {
            get {
                return defaultSchemaName == string.Empty ? "[dbo]" : $"[{defaultSchemaName}]";
            }
            set {
                defaultSchemaName = value;
            }
        }
        public virtual IDatabaseObject DeriveDatabaseObject => DatabaseObjectInstance;
        ...
        public GenericDataStore(runtime.MemoryCache cache) : this() {
            _runtimeCache = cache;
        ...
        public GenericDataStore(IMemoryCache cache) : this() {
            _cache = cache;
        ...
        public GenericDataStore(runtime.MemoryCache cache, Assembly callingAssembly, DatabaseConnection databaseToUse) : this(cache) {
            ...
            DatabaseObjectInstance = DatabaseObject.DatabaseObjectFactory(databaseToUse, cache);
        ...
        /// <param name="callingAssembly"></param>
        /// <param name="databaseToUse"></param>
        public GenericDataStore(IMemoryCache cache, Assembly callingAssembly, DatabaseConnection databaseToUse) : this(cache) {
            // The calling assembly should be the assembly with the data aware classes to load into the ORM.
            if (ORM.Count == 0) {
                LoadAssemblyAndORM(callingAssembly);
            }
        ...
        public GenericDataStore(IMemoryCache cache, Assembly callingAssembly, ConnectionSetting connectionSetting) : this(cache) {
            ...
            TransactionInProgress = false;
            DatabaseObjectInstance = DatabaseObject.DatabaseObjectFactory(connectionSetting, cache);
            DatabaseObjectInstance.CommandTimeout = 0;
            ConnectionSetting = connectionSetting;
            DatabaseToUse = new(connectionSetting);
        }
        /// <summary>
        /// Initializes a new instance of the GenericDataStore class. The internal ORM model (shared between all instances of the GenericDataStore class) is initialized to the calling assembly.
        /// </summary>
        /// <param name="cache"></param>
        /// <param name="databaseToUse"></param>
        public GenericDataStore(runtime.MemoryCache cache, DatabaseConnection databaseToUse) : this(cache, Assembly.GetCallingAssembly(), databaseToUse) {
        }
        /// <summary>
        /// Initializes a new instance of the GenericDataStore class. The internal ORM model (shared between all instances of the GenericDataStore class) is initialized to the calling assembly.
        /// </summary>
        /// <param name="cache"></param>
        /// <param name="databaseToUse"></param>
        public GenericDataStore(IMemoryCache cache, DatabaseConnection databaseToUse) : this(cache, Assembly.GetCallingAssembly(), databaseToUse) {
        }
        ...
        public GenericDataStore(runtime.MemoryCache cache, DatabaseConnection databaseToUse, string databaseSchemaName) : this(cache, databaseToUse) {
            DefaultSchemaName = databaseSchemaName;
            DatabaseObjectInstance.DefaultSchemaName = databaseSchemaName;
            DatabaseToUse.SchemaName = databaseSchemaName;
        }
        ...
        /// <param name="databaseToUse"></param>
        /// <param name="databaseSchemaName"></param>
        public GenericDataStore(IMemoryCache cache, DatabaseConnection databaseToUse, string databaseSchemaName) : this(cache, databaseToUse) {
            DefaultSchemaName = databaseSchemaName;
            DatabaseObjectInstance.DefaultSchemaName = databaseSchemaName;
            DatabaseToUse.SchemaName = databaseSchemaName;
        }
        ...
        /// <summary>Parses an assembly for ORM mappings.</summary>
...
#pragma warning restore IDE1006 // Naming Styles
        ...
        protected void ExecuteBulkCopy_SqlRowsCopied(object sender, SqlRowsCopiedEventArgs e) {
            BulkCopySqlRowsCopied?.Invoke(sender, e);
        }
        ...
        public Boolean TestConnection() {
            return DatabaseObjectInstance.TestConnection();
        }
        ...
        public String GetViewNameFromObject<T>() {
            DataTableAttribute table = (DataTableAttribute)ORM[typeof(T).FullName][typeof(T)];
            if (table.ViewName == String.Empty) { return GetTableNameFromObject<T>(); }
            if (String.IsNullOrEmpty(table.SchemaName))
                if (!String.IsNullOrEmpty(DefaultSchemaName))
                    return $"{DefaultSchemaName}.{table.TableName}";
        ...
        public string GetViewNameFromObject(string typeName) {
            DataTableAttribute table = (DataTableAttribute)ORM[typeName][Type.GetType(typeName)];
            if (table.ViewName == String.Empty) { return GetTableNameFromObject(typeName); }
            if (String.IsNullOrEmpty(table.SchemaName))
                if (!String.IsNullOrEmpty(DefaultSchemaName))
                    return $"{DefaultSchemaName}.{table.TableName}";
            return table.ViewName;
        ...
        protected void MapApiColumns(Type type, ApiDataControllerAttribute attrApi) {
            ...
            try {
                foreach (PropertyInfo pi in properties) {
                    ...
                    if (param != null) {
                        if (attrApi.Properties == null) attrApi.Properties = new();
