...
namespace CodedThought.Core.Data {
	...
	public class GenericApiDataStore : GenericDataStore {
		...
		/// <returns></returns>
		internal string EncodeBasicAuthCredentials(string username, string password) => Convert.ToBase64String(System.Text.ASCIIEncoding.UTF8.GetBytes($"Basic {username}:{password}"));
		/// <summary>
		...
		private void PopulateCurrentConnection(ConnectionSetting? connectionSetting) {
			if (connectionSetting == null)
				throw new ArgumentNullException("The ConnectionSetting cannot be null.");
		...
		protected T DeserializeApiResponse<T>(ApiDataReader reader) {
			dynamic objectJson = JsonConvert.SerializeObject(reader.Data);
			return JsonConvert.DeserializeObject<T>(objectJson);
		}

		protected T DeserializeApiResponse<T>(string response) => JsonConvert.DeserializeObject<T>(response);
